...
if ready:
    ...
    stage("immersive-regression",[sys.executable,str(root/"tests/immersive_regression.py"),"--binary",str(app),"--output",str(out/"immersive-regression")],780,env)
    ...
    if a.offline:
        ...
        if not pathlib.Path(py).exists():
            ...
            stage('online-artwork-live',['python3',str(root/'tests/online_artwork_live.py'),'--output',str(out/'online-artwork-live'),'--binary',str(app)],1500,e)
            for name,flag in [('native-features','--features-test'),('live-lyrics-motion','--lyrics-test'),('playback-recovery','--recovery-test'),('ui-playback','--ui-test'),('ui-audit','--audit')]:
                ...
                stage(name,[str(app),'--isolated',flag],480,e)
            ...
            stage('ui-hidpi',[str(app),'--isolated','--ui-test'],480,e)
